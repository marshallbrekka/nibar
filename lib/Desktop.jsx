...
function base64ToImageObject(base64) {
  let bytes = convertDataURIToBinary(base64);
...
const firefoxBlob = base64ToImageObject(firefox);
const blobUrl = URL.createObjectURL(firefoxBlob);
...
const renderSpace = ({index, focused, visible, windows, label}) => {
  ...
  if (index == 6) {
    return (
      <div key={label} style={contentStyle}><img style={imageStyle} src={blobUrl} /></div>
    );
  }
  return (
